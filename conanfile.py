...
class GTestConan(ConanFile):
    ...
    def generate(self):
        ...
        if self.settings.build_type == "Debug" and Version(self.version) < "1.12.0":
            tc.cache_variables["CUSTOM_DEBUG_POSTFIX"] = str(
                self.options.debug_postfix)

        if self.settings.compiler.get_safe("runtime"):
            tc.variables["gtest_force_shared_crt"] = "MD" in msvc_runtime_flag(
                self)
    ...
    def package_info(self):
        ...
        if self.settings.os in ["Linux", "FreeBSD"]:
            ...
            if not self.options.disable_pthreads:
                self.cpp_info.components["libgtest"].system_libs.append(
                    "pthread")
